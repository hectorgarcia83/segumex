<div class="altaseguridad">
	<div class="row">
		<div class="medium-12 medium-centered columns iconos-formasdepago">
			<div class="small-6 medium-4 large-2 large-offset-1 columns text-center altaseguridad-elemento">
				<div class="icon-efectivo">
					<img src="media/images/icono_huella.png" alt="">
				</div>
				<div class="texto-tipopago">
					HUELLA DIGITAL
				</div>
			</div>

			<div class="small-6 medium-4 large-2 columns text-center altaseguridad-elemento">
				<div class="icon-efectivo">
					<img src="media/images/icono_camara.png" alt="">
				</div>
				<div class="texto-tipopago">
					CÁMARA DE VIGILANCIA
				</div>
			</div>

			<div class="small-6 medium-4 large-2 columns text-center altaseguridad-elemento">
				<div class="icon-efectivo">
					<img src="media/images/icono_codigo.png" alt="">
				</div>
				<div class="texto-tipopago">
					CLAVE DE SEGURIDAD
				</div>
			</div>

			<div class="small-6 medium-4 large-2 columns text-center altaseguridad-elemento">
				<div class="icon-efectivo">
					<img src="media/images/icono_vigilancia.png" alt="">
				</div>
				<div class="texto-tipopago">
					VIGILANCIA
				</div>
			</div>

			<div class="small-6 medium-4 large-2 columns end text-center altaseguridad-elemento">
				<div class="icon-efectivo">
					<img src="media/images/icono_llave.png" alt="">
				</div>
				<div class="texto-tipopago">
					LLAVE DE SEGURIDAD
				</div>
			</div>
			
		</div>
	</div>
	<div class="row">
		<div class="small-10 medium-9 large-8 columns small-centered medium-centered large-centered text-center">
			<div class="planes-facilidades">PREGUNTA POR NUESTROS PLANES Y FACILIDADES DE PAGO</div>
		</div>
	</div>
</div>
<div class="row text-center siguiente">
	<a href="javascript:goToSection(3)" >
		<img src="media/images/icono_flecha-abajo.png" alt="">
	</a>
</div>